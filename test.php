...
function initRedis(){
    ...
    function token_wx(){
        $access_token['access_token']= 12312444444444444;
        return $access_token;
    }
...
{
    $redis=initRedis();
    $token_time=$redis->hget('token','time');
    if ($token_time==false) {
        $time=time();
        $redis->hset('token','time',$time);
        return _accesstoken();
    }else{
        $now=time();
        $diff=$now-$token_time;
        if ($diff>3600) {
            $access_token=token_wx();
            $time=time();
            $redis->hset('token','time',$time);
            $redis->hset('token','token',$access_token['access_token']);
        }
        $token=$redis->hget('token','token');
        if ($token==false) {
            $access_token=token_wx();
            $time=time();
            $redis->hset('token','time',$time);
...
/*$redis->hset('port:123','emp','12312');




$redis->hset('port:123','emp','12312111111');
var_dump($redis->hget('port:123','emp'));
var_dump($redis->del('port:123'));
var_dump($tmp=$redis->hget('port:123','token'));*/
if(empty($emp_open)) {
        var_dump(1);
    }else{
        var_dump(2);
